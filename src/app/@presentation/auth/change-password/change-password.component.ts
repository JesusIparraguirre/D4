...
export class ChangePasswordComponent implements OnInit {
  ...
  private identificadorCambioContrasenia: string;
  ...
  onSubmit() {
    ...
    this.showLoading();
    const request: RequestGenerico[] = [
      {
        llave: 'IdentificadorCambioContrasenia',
        valor: this.identificadorCambioContrasenia
      },
      {
        llave: 'Clave',
        valor: this.f.actualpass.value
      },
      {
        llave: 'NuevaClave',
        valor: this.f.pass.value
      },
    ];
    this.authserv.ChangePAss(request)
      .subscribe(result => {
        this.hideLoading();
        if (result) {
          this.restoreOk = true;
        } else {
          Utils.showToast('Error al cambiar la contraseña', this.toastrService, 'danger');
        }
      }, error => {
        this.hideLoading();
  ...
  ngOnInit(): void {
    ...
    this.identificadorCambioContrasenia = this.route.snapshot.params['id-change-pass'];
    if (!this.identificadorCambioContrasenia) {
  ...
  logout() {
    this.user = this.authSerivces.getCurrentUserValue;

    if(this.user != undefined)
      this.authSerivces.logout(this.user.id, true);
  }
